...
public class EmpDAO {
	...
	public ArrayList<MovieVO> movieListData()
	{
		...
		{
			getConnection();
			String sql="SELECT no,title,poster,director,actor,genre,story FROM daum_movie ORDER BY no";
	...
	public MovieVO movieDetalData(int no)
	{
		MovieVO vo=new MovieVO();
		try 
		{
			getConnection();
			String sql="SELECT no,title,poster,director,actor,genre,story FROM daum_movie "
					+ "WHERE no="+no;
