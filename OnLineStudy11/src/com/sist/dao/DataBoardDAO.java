...
public class DataBoardDAO {
   ...
   static 
   {
	   try
	   {
		   //xml 파일 읽어서 (reader -> xml에 한글 있어서) 읽은 파일을 파싱해서 ssf에 넣어놓는
		   //ssf=new sqlseesionfactorybuilder().build(reader)
   ...
   public static List<DataBoardVO> boardListData(Map map)
   {
	   ...
	   try
	   {
		   ...
		   // 2. mapper.XML에 있는 SQL문장을 실행후에 결과값을 달라
   ...
   public static void boardInsert(DataBoardVO vo)
   {
	   ...
	   try 
	   {
		   ...
		   session=ssf.openSession(true); // commit 자동수행
		   // INSERT,UPDATE,DELETE 반드시 commit 수행
